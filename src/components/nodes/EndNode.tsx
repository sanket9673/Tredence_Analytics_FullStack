...
export const EndNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  const typedData = data as unknown as EndNodeData
  const { highlightedNodeId } = useWorkflowStore()
  const isHighlighted = highlightedNodeId === id
  return (
    <div className={`custom-node node-end ${selected ? 'selected' : ''} ${isHighlighted ? 'ring-4 ring-offset-2 ring-violet-400 animate-pulse' : ''}`}>
      ...
      <div className="custom-node-body">
        ...
        <div className="custom-node-meta">
          Summary: {typedData.summaryFlag ? 'On' : 'Off'}
        </div>
      </div>
