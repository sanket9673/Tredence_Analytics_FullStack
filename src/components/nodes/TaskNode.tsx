...
export const TaskNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  return (
    <div className={`custom-node node-task ${selected ? 'selected' : ''} ${isHighlighted ? 'ring-4 ring-offset-2 ring-violet-400 animate-pulse' : ''}`}>
      ...
      <div className="custom-node-body">
        ...
        <div className="custom-node-meta flex justify-between">
          <span>{typedData.assignee || 'Unassigned'}</span>
          <span>{typedData.dueDate || 'No date'}</span>
        </div>
      </div>
